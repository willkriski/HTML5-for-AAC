function EditCtrl($scope, categoryService){
	
	$scope.imageGrid = categoryService.data;
...
function GridCtrl($scope, $location, categoryService){
	
    $scope.wordGrid = categoryService.data;
	       ...
	       $scope.say = function(text){
			...
			+voice+"&AUDIO=WAVE_FILE");
  			audioword.play();
  			$location.path('/word/' + text);

	};
}
function WordCtrl($scope, wordService, $routeParams){
	
    $scope.wordGrid = wordService.data;
...
GridCtrl.$inject = ['$scope', '$location', 'categoryService'];
EditCtrl.$inject = ['$scope', 'categoryService'];
WordCtrl.$inject = ['$scope', 'wordService', '$routeParams'];